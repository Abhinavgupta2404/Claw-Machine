const elements = {
  ...
  /**
   * Represents a vertical button with specific actions for a claw machine.
   * 
   * @type {Button}
   * @property {string} className - The CSS class name for the button.
   * @property {boolean} isLocked - Indicates whether the button is locked and cannot be pressed.
   * @property {Function} pressAction - The action to perform when the button is pressed.
   * @property {Function} releaseAction - The action to perform when the button is released.
   * 
   * pressAction:
   * - Moves the arm joint along the 'y' axis to the target position in the machine buffer.
   * - Does nothing if the button is locked.
   * 
   * releaseAction:
   * - Stops the arm joint movement and deactivates the button.
   * - Finds the closest toy and performs a sequence of actions:
   *   1. Opens the claw.
   *   2. Moves the arm to the maximum length.
   *   3. Closes the claw to grab the toy.
   *   4. Resumes movement of the arm and rails to drop the toy.
   */
